...
void input_completed(GtkWidget *entry, gpointer data)
{
    ...
    if (strcmp(text, password) == 0)
        gtk_label_set_text(GTK_LABEL((GtkWidget *)data), "LOGGIN!\n");
    else
        gtk_label_set_text(GTK_LABEL((GtkWidget *)data), "ERROR!\n");
}
...
int main(int argc, char *argv[])
{
    ...
    gtk_window_set_title(GTK_WINDOW(window), "add information");
    ...
    message_label = gtk_label_new("ID/PW.");
